...
export async function saveSettings(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  // not come back as "Saved." See `wroteNoRows` for why only a definite zero qualifies.
  if (wroteNoRows(count)) return { ok: false, error: 'Could not save your settings. Try again.' };
